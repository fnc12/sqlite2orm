...
**Statement map (SQL ↔ generated code):** every assembly of more than one statement reports which statement each stretch of its code came from — `CodeGenResult::spans` for `generateSqliteSchemaHeader`, `joinGeneratedCodeWithSpans` for the batch behind the playground. A `GeneratedCodeSpan` names the offset and length in the generated code, the index of the statement it was generated from and that statement's place in the SQL it was parsed from; offsets and lengths count characters on both sides, as `CodegenWarning` does. A statement is on the map once per fragment it generated — a CREATE TABLE twice, for its struct and for its `make_table(...)` argument — and a statement codegen threw away is on it by nothing at all. In the `functional` savepoint style a matched RELEASE is on it by the lines closing the lambda it ends. The list is statement level for good and its spans never overlap; a finer map, down to expressions, will come as a list of its own.

**Schema pipeline (phases 21.3–21.7):** `schema_process.h` (`processSqliteSchema`), `schema_header.h` (`generateSqliteSchemaHeader`), `codegen_policy.h` + `processSql(..., const CodeGenPolicy*)`, `json_emit.h` (`sqliteSchemaResultToJson`, via **nlohmann/json** single header downloaded at CMake configure). CLI: `sqlite2orm --db path.db` (header), `--db path.db --json`; both render through `schema_report.h` (`reportSqliteSchema`), so a schema with a statement that did not generate prints its diagnostics on stderr and exits 1 in either mode. That statement is left out of `make_storage()` with a warning rather than taking the header with it — SQLite only compiles a view or trigger body when the object is used, so it stores bodies sqlite2orm refuses — and the name it created joins the ungeneratable set so nothing resting on it is emitted with a dangling reference. Merged header uses `using namespace sqlite_orm` inside `make_sqlite_schema_storage`; `CREATE VIRTUAL TABLE` is warned and omitted from `make_storage`. Both this header and the batch merge behind the playground (`joinGeneratedCode`) write a storage's indexes and triggers before the tables and views they are made for, through one rule: sqlite_orm up to and including v1.9.1 syncs the database objects of a storage in reverse declaration order, so the other order makes `sync_schema()` throw `no such table` against that release, while revisions after it sort the objects by dependency and take either one.
...
- [x] numeric-literal (real / float) — a literal past the range of a double is an Inf to SQLite and ill-formed in C++, so it is spelled `std::numeric_limits<double>::infinity()` (one below the range, the `0.0` it rounds to), and the generated header takes `<limits>` along. A query carries the value bound, so it reaches SQLite there as the number it is; the clauses sqlite_orm writes into the DDL it creates a schema object with — a column `DEFAULT` or `CHECK`, a generated column, a table `CHECK`, an index, a trigger, a view — are generated and warned about instead, because sqlite_orm's serializer writes an infinity as `inf`, which SQLite reads as a column name: `sync_schema()` throws over a table or an index, and creates a trigger or a view that refuses every use of it (upstream sqlite_orm owns the spelling; the warning goes when it writes one SQLite reads as a number)
...
- [~] `IN (expr-list)` — generated as `in(operand, {...})`, one braced-init-list, and sqlite_orm declares it `in(A, std::initializer_list<E>)`: every value has to reach C++ as one type, by the same rule the bounds of a BETWEEN are under. Integer values of two widths are given one by a `static_cast<int64_t>` on each of them (`x IN (1, 3000000000)`, `x IN (1, 2, 0xDEADBEEF)`), and an empty list, which has no value to deduce `E` from at all, is generated as `std::vector<int64_t>{}` — sqlite_orm serializes it to the `IN ()` SQLite reads as always false. A list of `bool` values takes the same cast although it is one type already: the list is collected into a `std::vector<E>`, and a `std::vector<bool>` is the one vector the walk over a statement's bound values cannot read, so `in(&User::a, {true})` builds on its own and fails the moment a statement holds it. A bind parameter beside the values stops neither widening — it is typed by the caller, and the `int64_t` he declares is what the cast on the values beside it meets (`a IN (TRUE, ?)`, `a IN (1, ?, 3000000000)`). Partial because values with nothing to widen to have no working form — an integer beside a real, a string, a blob, a NULL, a column or a nested expression — and the generated code does not compile: codegen warning, naming the pair of values that proves it. Silent where nothing here types the values: columns alone, expression nodes alone, and a bind parameter the caller declares, which is not cast with the constants beside it either. Independent of the field the prefix infers, which widens to the widest value
- [x] `IN (select-stmt)`
...
- [x] CASE without ELSE
- [~] result type — `case_<R>` reads every row of the column through the one `R`, while SQLite answers a CASE with the value of whichever branch matched, so `R` is the widest type over all the branch results and the ELSE (the operand of a simple CASE is compared against rather than answered with, so it is not one of them): `bool` widens to `int`, `int` to `int64_t` and to the `double` that holds every int32 exactly, and anything to `std::string`, which reads back every storage class. An `int64_t` branch beside a REAL one is the pair with no number over it — a `double` drops every integer past 2^53, an `int64_t` the fractional part of a REAL — so that pair widens to text as well: `CASE WHEN a > 0 THEN a * 1 ELSE 1.5 END` is generated as `case_<std::string>().when(c(&T::a) > 0, then(c(&T::a) * 1)).else_(1.5).end()`, which reads both back as SQLite prints them. A branch naming a column of a CREATE TABLE of the batch contributes the type of the field that column is read into — `CASE WHEN age > 1 THEN name ELSE NULL END` over a TEXT `name` is `case_<std::string>`, over an INTEGER one `case_<int64_t>`, over a REAL one `case_<double>` — a NULL branch contributes nothing but the `as_optional`, and a BLOB column or blob literal beside anything else is read through the `std::vector<char>` that keeps every byte of it, an `std::string` stopping at the first NUL. A column no CREATE TABLE of the batch names (a query generated on its own, a column of a CTE or a view) is still read through the default `int`. A branch whose type the operation knows rather than the literal under it — a concatenation, a CAST, a function call, a JSON arrow — is still read through the `int` the inference answers by default, so `CASE WHEN a THEN a || 'x' ELSE 1 END` comes back as 7 where SQLite answers `7x`
- [x] a view column of a CASE — the field a `make_view` struct holds for the column is widened over the branches and the ELSE the same way, and holds an `std::optional` as soon as a branch spells a NULL out or the CASE has no ELSE to answer with; a BLOB branch beside a non-BLOB one is an `std::vector<char>`, since an `std::string` stops at the first NUL byte a blob holds — the same type the `case_<R>` of that column is generated with

### CAST
- [x] `CAST(expr AS type-name)`
- [x] Multi-word type names (`UNSIGNED BIG INT`)
- [x] Type with size spec (`VARCHAR(255)`, `DECIMAL(10, 2)`)
- [x] The C++ type a CAST is read back through follows SQLite's affinity rule for the type name, the same rule a column's declared type goes by except for `BOOLEAN`: a column declared BOOLEAN maps to a `bool` field, but a CAST to BOOLEAN converts to the NUMERIC affinity the name falls through to and never down to 0 or 1 — sqlite3 3.51 and 3.45.1 answer `CAST(7 AS BOOLEAN)` with 7 and `CAST(1.5 AS BOOLEAN)` with 1.5 — so it is generated as `cast<double>` like every other NUMERIC name, where the `cast<bool>` it used to be read the 7 back as 1. A name holding `INT` stays INTEGER whatever else it holds (`CAST(1.5 AS BOOLINT)` is 1, `cast<int64_t>`)

### Subquery expression
- [~] `(select-stmt)` as scalar subquery — sqlite_orm parenthesizes a nested `select_t` (`COALESCE((SELECT "u"."b" FROM "u"), 1)`), so a plain subquery stands wherever an expression does. A COMPOUND one does not: `union_()` / `union_all()` / `intersect()` / `except()` build a statement, whose serializer writes no parentheses of its own, so the same subquery came out as `COALESCE(SELECT … UNION SELECT …, 1)`, `"t"."a" > SELECT …`, `EXISTS SELECT …` and `ORDER BY SELECT …` — SQL sqlite3 3.51 answers with `SQL logic error` — and as a result column beside a FROM it did not compile at all (`static_assert(… "Cannot use args with a compound operator")`). The two positions the enclosing form does bring the parentheses for are the whole condition of a `where(...)`, which serializes as `WHERE (…)`, and the right-hand side of `in(...)`; `having(...)`, `on(...)`, `order_by(...)` and every value slot write their argument bare. So a compound subquery is generated in those two and leaves the statement out with a codegen warning anywhere else, one level down from a WHERE included — `and_` writes its operands bare too. Partial for that reason. A plain subquery standing as the LEFT operand of a binary operator is quoted with `c()` the way a literal is: sqlite_orm declares its operators for an operator argument on at least one side and `select_t` is none, so `SELECT (SELECT b FROM u) + 1 FROM t` comes out as `c(select(&U::b)) + 1` — the same for `-`, `*`, `||`, `>`, `=` and `&` — and a subquery on the RIGHT, beside a wrapped left operand (`c(&T::a) > select(…)`), builds as it is. A plain subquery is otherwise no obstacle to a call that reads its result type off its arguments: `coalesce(select(&U::b), 1)` builds, with a NATURAL JOIN inside the subquery as well. What does not build there is a window function, subquery or no — `SELECT coalesce(sum(b) OVER (ORDER BY b), 1) FROM u` stops at `column_result_t<…over_t…>` having no `type` just the same — which is a gap of `over_t`, not of the subquery
...
- [~] `function-name(...) FILTER (WHERE expr)` → `.filter(where(...))` before `.over(...)` when present; sqlite_orm gives a `filter()` to `count(*)` and the aggregate function calls only, so a FILTER over anything else — a scalar function, a window function, MATCH in its function spelling, the argument-less `count()`, a user-defined function — has no form to generate the call as, and the call is placeheld with a warning underlining it rather than written out into code that does not compile. SQLite refuses the same thing at prepare (`FILTER clause may only be used with aggregate window functions` over a window function, `FILTER may not be used with non-aggregate upper()` over a scalar one) while storing a trigger or a view that holds it
- [~] `function-name() OVER window-name` → `.over(window_ref("..."))`; sqlite_orm gives an `over()` to the window functions, to `count(*)` and to the aggregate function calls — and to the `filtered_aggregate_function` a `filter()` over one returns, so an aggregate takes a FILTER and an OVER at once — so an OVER over anything else is refused the same way a FILTER is, rather than written out as the `.over(...)` no such form declares. SQLite refuses the same call at prepare (`upper() may not be used as a window function`) while storing a trigger or a view that holds it
- [~] `function-name() OVER (window-defn)` — PARTITION BY, ORDER BY, ROWS|RANGE|GROUPS frame, EXCLUDE; the same forms carry it, and the same refusal covers every other one
...
common type")`, sqlite_orm reading the `sum` back as an `std::unique_ptr<double>` beside the
`std::optional<int64_t>` of `a`.

A compound SELECT standing as a scalar subquery is a separate matter: sqlite_orm has a form for it
only where the enclosing one supplies the parentheses its serializer omits — see the subquery
expression row above — and a raw insert has none for it at all
(`insert(into<U>(), columns(&U::b), union_(…))` trips `static_assert(… "Raw insert has invalid
arguments")`), so `INSERT INTO u(b) SELECT … UNION SELECT …` is placeheld whole.

### WITH (CTE)
- [~] WITH cte AS (select-stmt) — a subquery standing as a WHOLE column of a CTE has no sqlite_orm form: the columns of a CTE are read through `extract_colref_expressions()`, whose overload for a `select_t` is deleted, so `cte<cte_0>().as(select(select(&U::b)))` does not compile and the statement is left out with a codegen warning instead. That slot alone: the same subquery in the CTE's own WHERE, or under a CAST or a call inside the column, compiles and is generated. Under a binary operator inside the column it is generated as well, quoted with `c()` as the left operand of an operator is everywhere, see the subquery expression row. A select reading a CTE also spells its FROM out as `from<cte_0>()` wherever leaving it implicit would get it wrong, by the same rule an ordinary select is under: a subquery in the WHERE named its own table and sqlite_orm collected it into this FROM as a second source (`WITH recent AS (SELECT id, name FROM users) SELECT name FROM recent WHERE id IN (SELECT uid FROM orders)` answered the four rows of the product where SQLite answers two), and a select naming no recordset of its own left sqlite_orm nothing to infer a FROM from at all (`SELECT 1 FROM recent` answered one row where SQLite answers three)
