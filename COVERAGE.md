...
a compound of bitwise branches is still read back through `int`. A branch holding a subquery whose
result type the codegen cannot name — `SELECT a FROM t UNION SELECT (SELECT sum(b) OVER (ORDER BY b)
FROM u LIMIT 1)` — has no common type to share either, and still takes the statement down with
`static_assert(… "Compound select statements must return a common type")`.
