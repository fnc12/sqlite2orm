...
- [x] Comma-separated table-refs (parsed as implicit `CROSS JOIN`; codegen emits join chain). SQLite reads an ON or a USING after a comma the way it reads one after JOIN, and so does the parser: `FROM users, posts ON users.id = posts.user_id` answers the rows the constraint keeps
...
- [x] LEFT JOIN
- [x] LEFT OUTER JOIN
- [x] CROSS JOIN. One carrying an ON or a USING is generated as `join<T>(...)` and warns: `cross_join_t` takes no constraint at all, and a CROSS JOIN answers the rows of an inner join — it differs from a JOIN only in that SQLite will not reorder the tables, which is what the warning is about
- [x] NATURAL JOIN
- [x] NATURAL LEFT JOIN
