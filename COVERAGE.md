...
- [x] INSERT INTO table VALUES (...) — the object form `storage.insert(T{...})`, except when a value cannot pass through the field it would reach in a table this batch declared: a decimal integer literal past the int64 range (`99999999999999999999`) and any REAL literal (`1.5`, `2.0`) reaching a field that holds whole numbers only (`int64_t` and `int`, and `bool` for a BOOLEAN column) make the statement spell its column list out instead — `storage.insert(into<T>(), columns(&T::x), values(std::make_tuple(99999999999999999999.0)))` — which binds the value and leaves the affinity to SQLite, exactly as the SQL does; SQLite types a value before it applies the column affinity, so it stores `real|1.0e+20` and `real|1.5` in an INTEGER column while the field would store a different number altogether. A generated column is left out of that column list, as SQLite computes it, whichever of `AS (...)`, `... VIRTUAL`, `... STORED` and `GENERATED ALWAYS AS (...)` the DDL spells. The past-range literal carries a warning naming the column and the form; a REAL literal does not, because the bound statement stores exactly what SQLite stores for it (`1.5` stays `real|1.5`, `2.0` becomes `integer|2`) and only the shape of the generated call changes. Left alone: a table this batch never declared (nothing is known about the field), `INSERT ... SELECT`, the explicit-column form above and a `DEFAULT` clause, which all bind the value already. An ordinary whole number reaching a `bool` field is a separate gap this does not close: `VALUES (5)` into a BOOLEAN column keeps the object form and writes 1, where SQLite stores `integer|5`
