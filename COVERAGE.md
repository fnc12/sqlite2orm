...
- [x] changes()
- [x] char(X1,X2,...,XN)
- [~] coalesce(X,Y,...) — sqlite_orm types the call as the common C++ type of its arguments (`common_argument_type<>`), and arguments with no common type — a text next to a number, a BLOB next to anything else — took the `storage.select(...)` around the call down with them (`no type named 'type' in 'column_result_t<…>'`). Where the generated types say there is none, the call spells the type it is read back through instead: `std::vector<char>` when a BLOB takes part, which carries every byte of one, and `std::string` otherwise, which carries every storage class as its text. What that costs is reported at the result column, the only place a type is read back at all — a number comes back as its digits: codegen warning. Partial because only a constant and a column the batch's schema types are answered for; an argument sqlite_orm types out of what it is built over — a nested call, an operator, a CAST, a bind parameter, a column no schema names — is left as it is generated, so `coalesce(abs(a), 'x')` still does not compile
