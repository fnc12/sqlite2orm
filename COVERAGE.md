...
- [ ] a `+`, `-`, `*`, `/` or `%` result column is read back through sqlite_orm's `double`, so an INTEGER result past 2^53 comes back rounded: `SELECT a + 0` over `a = 9223372036854775807` hands the caller 9223372036854775808. Nothing in sqlite_orm reads such a column as an int64 without a CAST, and a CAST to INTEGER cannot be added the way the bitwise one can: these operators answer a REAL as soon as an operand is one (checked over 1120 operand pairs against sqlite3 3.51) and the CAST would truncate it, and an overflowing `*` answers a REAL as well. So the column is generated as it was and carries a warning naming the operator, anchored at its token. The warning is left out where the SQL bounds the result inside the range a double holds exactly: a spelled-out operand bounds `+`, `-` and `*`, a divisor bounds `%`, a dividend bounds `/`, and a REAL or a NULL operand bounds the whole expression, because a double carries those as they are. Closing it needs a type-only integer wrapper in sqlite_orm, the way `as_optional` is one for nullability
