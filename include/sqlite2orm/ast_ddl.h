...
namespace sqlite2orm {
    ...
    struct CreateTableNode : AstNode {
        ...
        CreateTableNode(std::string tableName,
            ...
            AstNode(location), tableName(std::move(tableName)), columns(std::move(columns)), ifNotExists(ifNotExists) {}
        ...
        CreateTableNode(std::string tableName,
                        ...
                        bool ifNotExists,
                        SourceLocation location) :
            AstNode(location), tableName(std::move(tableName)), columns(std::move(columns)),
            foreignKeys(std::move(foreignKeys)), ifNotExists(ifNotExists) {}

        bool operator==(const AstNode& other) const override {
            auto* o = dynamic_cast<const CreateTableNode*>(&other);
            return o && this->tableName == o->tableName && this->columns == o->columns &&
                   this->foreignKeys == o->foreignKeys && this->primaryKeys == o->primaryKeys &&
                   this->uniques == o->uniques && this->checks == o->checks && this->ifNotExists == o->ifNotExists &&
                   this->withoutRowid == o->withoutRowid && this->strict == o->strict;
        }
    };
    ...
    struct CreateTriggerNode : AstNode {
        ...
        bool operator==(const AstNode& other) const override {
            ...
            if (!astNodesEqual(this->whenClause, o->whenClause))
                return false;
            if (this->bodyStatements.size() != o->bodyStatements.size())
                return false;
            for (size_t i = 0; i < this->bodyStatements.size(); ++i) {
                if (!astNodesEqual(this->bodyStatements.at(i), o->bodyStatements.at(i)))
                    return false;
            }
            return true;
        }
    ...
    struct IndexColumnSpec {
        ...
        bool operator==(const IndexColumnSpec& other) const {
            return astNodesEqual(this->expression, other.expression) && this->sortDirection == other.sortDirection &&
    ...
    struct CreateViewNode : AstNode {
        ...
        CreateViewNode(SourceLocation location,
            ...
            viewName(std::move(viewName)), columnNames(std::move(columnNames)), selectQuery(std::move(selectQuery)),
    ...
    struct AlterTableStatementNode : AstNode {
        ...
        AlterTableStatementNode(SourceLocation location,
                                std::optional<std::string> tableSchemaName,
                                std::string tableName) :
    ...
    struct SavepointNode : AstNode {
        ...
        SavepointNode(SourceLocation location, std::string name) : AstNode(location), name(std::move(name)) {}
    ...
    struct ReleaseNode : AstNode {
        ...
        ReleaseNode(SourceLocation location, std::string name) : AstNode(location), name(std::move(name)) {}
    ...
    struct PragmaNode : AstNode {
        ...
        bool operator==(const AstNode& other) const override {
            ...
            return o && schemaName == o->schemaName && pragmaName == o->pragmaName && astNodesEqual(value, o->value);
        }
    };
