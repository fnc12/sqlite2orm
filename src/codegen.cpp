...
namespace sqlite2orm {
    ...
    CodeGenResult CodeGenerator::tryCodegenSqliteSelectSubexpression(const SelectNode& selectNode) {
        ...
        return this->withRecordedComments(
            this->selectCodeGenerator->tryCodegenSqliteSelectSubexpression(selectNode), mark);
    }

    CodeGenResult CodeGenerator::tryCodegenCompoundSelectSubexpression(const CompoundSelectNode& compoundNode) {
        const size_t mark = this->generatorContext->commentMark();
        return this->withRecordedComments(
            this->selectCodeGenerator->tryCodegenCompoundSelectSubexpression(compoundNode), mark);
    }
    ...
    CodeGenResult CodeGenerator::generateTriggerStep(const AstNode& statement,
        ...
        const size_t mark = this->generatorContext->commentMark();
        return this->withRecordedComments(
            this->dmlCodeGenerator->generateTriggerStep(statement, subjectTableStruct), mark);
    }
