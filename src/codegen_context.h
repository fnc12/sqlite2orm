...
namespace sqlite2orm {
    ...
    class CodeGeneratorContext {
      public:
         ...
         *  column, a DEFAULT — found no declaration for. The expression is generated by a walk with
         *  no way to report a failure, so `clauseColumnMember` records the name here instead; the one
         *  site that generates such an expression takes them with `takeUnresolvedClauseColumns()` and
         *  answers with a diagnostic rather than with the clause.
         */
        std::vector<std::string> unresolvedClauseColumns;
         ...
         *  which is generated by a walk that cannot fail: a name no declaration answers is recorded
         *  in `unresolvedClauseColumns` and answered with itself, so that the site that asked for
         *  the expression can leave the clause out. Outside a CREATE TABLE the name itself.
         */
        std::string clauseColumnMember(std::string_view columnName);

        /** The names `clauseColumnMember` found no declaration for, taken out and cleared. */
        std::vector<std::string> takeUnresolvedClauseColumns();
