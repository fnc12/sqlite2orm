...
namespace sqlite2orm {
    ...
    CodeGenResult ExpressionCodeGenerator::generateExpression(const AstNode& astNode) {
        ...
        if (auto* binaryOp = dynamic_cast<const BinaryOperatorNode*>(&astNode)) {
            ...
            std::string wrappedLeft = (leftLeaf && !leftNoWrap && !nodeGeneratesColumnPointer(binaryOp->lhs.get()))
                                          ? wrap(leftResult.code)
                                          : leftResult.code;
            std::string wrappedRight = (rightLeaf && !rightNoWrap && !nodeGeneratesColumnPointer(binaryOp->rhs.get()))
            ...
            if (unaryOp->unaryOperator == UnaryOperator::minus) {
                if (negationForm == NegationForm::zeroMinusSubtraction) {
                    ...
                    operandResult.warnings.push_back(CodegenWarning{
                        "unary minus over a predicate (" + std::string(sqlPredicateLooserThanMinus(*unaryOp->operand)) +
            ...
            return CodeGenResult{"is_null(" + operandResult.code + ")",
            ...
            if (!inNode->tableName.empty()) {
                ...
                if (cteIt != this->context.activeCteTypedefByTableKey.end()) {
                    ...
                    std::string code = inFunc + "(" + operandResult.code + ", select(" + selectColumnCode + "))";
            ...
            if (inNode->subquerySelect) {
                ...
                std::string code = inNode->negated ? "not_in(" + operandResult.code + ", " + sub.code + ")"
