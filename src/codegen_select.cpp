...
namespace sqlite2orm {
    ...
    CodeGenResult SelectCodeGenerator::generateSelect(const SelectNode& selectNode) {
        ...
        // wrong in either of two ways — it would DIFFER from what the SQL asked for (a subquery in
        // the WHERE names its own tables, and sqlite_orm collects them into this FROM; code naming
        // no recordset at all leaves it with no FROM to infer), or it would LOSE an alias this
        // select has already named. The second half cannot be vetoed: the forms that took
        // `canNameInferredFromSources` up are already generated, and `count<alias_a<T>>()` carries
        // no table at all, so without the clause the select would go out with no FROM whatsoever.
        ...
        if (inferredFromWouldLoseAlias) {
            ...
            this->context.recordComment(kCommentAliasedFromSources);
                   ...
                   implicitFromDiffers(fromSources, this->context.emittedTableTypes)) {
