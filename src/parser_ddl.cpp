...
namespace sqlite2orm {
        ...
        while (!atEnd()) {
            if (parenthesisDepth == 0) {
                if (check(TokenType::comma) || check(TokenType::rightParen))
                    break;
                if (check(TokenType::kwPrimary) || check(TokenType::kwNot) || check(TokenType::kwUnique) ||
                    check(TokenType::kwCheck) || check(TokenType::kwDefault) || check(TokenType::kwCollate) ||
                    check(TokenType::kwReferences) || check(TokenType::kwGenerated) || check(TokenType::kwConstraint) ||
                    check(TokenType::kwAutoincrement) || check(TokenType::kwAs))
                    break;
                ...
                if (!keywordTypoSuggestion(current().value).empty())
                    break;
            }
    ...
    ForeignKeyAction DdlParser::parseForeignKeyAction() {
        if (match(TokenType::kwCascade))
            return ForeignKeyAction::cascade;
        if (match(TokenType::kwRestrict))
            return ForeignKeyAction::restrict_;
    ...
    AstNodePointer DdlParser::parseDefaultValue() {
        ...
        if (check(TokenType::plus) || check(TokenType::minus)) {
            auto op = check(TokenType::minus) ? UnaryOperator::minus : UnaryOperator::plus;
    ...
    void DdlParser::parseColumnConstraints(ColumnDef& columnDef) {
        while (!check(TokenType::comma) && !check(TokenType::rightParen) && !atEnd()) {
            ...
            } else if (check(TokenType::kwGenerated) ||
                       (check(TokenType::kwAs) && peekToken(1).type == TokenType::leftParen)) {
    ...
    AstNodePointer DdlParser::parseCreateViewTail(SourceLocation location, std::string_view headerText) {
        ...
        std::string viewName;
        if (!this->parser.parseDmlQualifiedTable(viewSchemaName, viewName))
            return nullptr;
    ...
    AstNodePointer DdlParser::parseCreateTableTail(SourceLocation location) {
        ...
        // A key naming anything but a column — `PRIMARY KEY(1)`, or the expression SQLite answers
        // with `expressions prohibited in PRIMARY KEY and UNIQUE constraints` — is no table at all.
        if (keyColumnRefused)
            return nullptr;
        ...
        auto node = std::make_unique<CreateTableNode>(std::move(tableName),
                                                      std::move(columns),
    ...
    AstNodePointer DdlParser::parseCreateTriggerAfterKeyword(SourceLocation location, bool temporary) {
        ...
        if (!this->parser.parseDmlQualifiedTable(tableSchema, tableName))
            return nullptr;

        bool forEachRow = false;
        if (check(TokenType::kwFor)) {
            advanceToken();
            if (!match(TokenType::kwEach))
                return nullptr;
    ...
    bool DdlParser::parseIndexColumnSpec(IndexColumnSpec& out) {
        ...
        while (true) {
            ...
            if (match(TokenType::kwAsc)) {
                if (out.sortDirection != SortDirection::none)
                    return false;
                out.sortDirection = SortDirection::asc;
                continue;
            }
    ...
    AstNodePointer DdlParser::parseCreateIndexAfterKeyword(SourceLocation location, bool unique) {
        ...
        if (!check(TokenType::rightParen)) {
            ...
            if (!parseIndexColumnSpec(spec))
                return nullptr;
            columns.push_back(std::move(spec));
            while (match(TokenType::comma)) {
                IndexColumnSpec nextSpec;
                if (!parseIndexColumnSpec(nextSpec))
                    return nullptr;
