...
namespace sqlite2orm {
    ...
    DmlParser::DmlParser(Parser& parser, TokenStream& tokenStream) : parser(parser), tokenStream(tokenStream) {}
    ...
    bool DmlParser::parseUpsertTail(InsertNode& node, bool replaceInto) {
        ...
        if (match(TokenType::kwWhere)) {
            ...
            if (!node.upsertConflictWhere)
                return false;
        }
        if (!match(TokenType::kwDo))
            return false;
        if (match(TokenType::kwNothing)) {
            node.upsertAction = InsertUpsertAction::doNothing;
            return true;
        }
        if (!match(TokenType::kwUpdate))
            return false;
        if (!match(TokenType::kwSet))
            return false;
        if (!parseCommaSeparatedUpdateAssignments(node.upsertUpdateAssignments))
            return false;
        node.upsertAction = InsertUpsertAction::doUpdate;
        if (match(TokenType::kwWhere)) {
            node.upsertUpdateWhere = this->parser.parseExpression();
            if (!node.upsertUpdateWhere)
                return false;
        }
        return true;
    }
    ...
    AstNodePointer DmlParser::parseInsertStatement(bool replaceInto) {
        ...
        if (check(TokenType::kwDefault)) {
            advanceToken();
            if (!match(TokenType::kwValues))
                return nullptr;
            node->dataKind = InsertDataKind::defaultValues;
            if (!parseUpsertTail(*node, replaceInto))
                return nullptr;
            if (!parseReturningClause(node->returning))
                return nullptr;
            return node;
        }
        ...
        do {
            ...
            if (match(TokenType::star)) {
                ...
                if (match(TokenType::kwAs)) {
                    ...
                    advanceToken();
                } else if (isColumnNameToken() && !check(TokenType::comma) && !check(TokenType::semicolon) &&
                           !atEnd()) {
                    column.alias = std::string(current().value);
                    advanceToken();
                }
