...
namespace sqlite2orm {
    ...
    namespace {
        ...
        std::vector<CodeSpanBuilder> foldFunctionalSavepoints(std::vector<std::string> statements,
            ...
            std::vector<CodeSpanBuilder> out;

            auto appendStatement = [&](CodeSpanBuilder statement) {
                if (!stack.empty()) {
            ...
            for (size_t index = 0; index < statements.size(); ++index) {
                ...
                if (const auto* savepointNode = dynamic_cast<const SavepointNode*>(node);
                    ...
                    stack.push_back(OpenWrap{savepointNode->name, std::move(header), origin, {}});
                    continue;
                }
                if (const auto* releaseNode = dynamic_cast<const ReleaseNode*>(node);
                    ...
                    block.appendFragment(wrap.header, wrap.origin);
                    for (const CodeSpanBuilder& inner: wrap.innerStatements) {
                        block.append("\n");
            ...
            while (!stack.empty()) {
                OpenWrap wrap = std::move(stack.back());
                stack.pop_back();
                std::vector<CodeSpanBuilder> unwound;
                const size_t nameStart = wrap.header.find('(');
                const size_t nameEnd = wrap.header.find(", [&] {");
                CodeSpanBuilder manualCall;
                manualCall.appendFragment("storage.savepoint" + wrap.header.substr(nameStart, nameEnd - nameStart) +
                                              ");",
                                          wrap.origin);
                unwound.push_back(std::move(manualCall));
                for (CodeSpanBuilder& inner: wrap.innerStatements) {
                    unwound.push_back(std::move(inner));
                }
                for (CodeSpanBuilder& statement: unwound) {
                    if (!stack.empty()) {
                        stack.back().innerStatements.push_back(std::move(statement));
                    } else {
                        out.push_back(std::move(statement));
    ...
    std::string joinGeneratedCode(const std::vector<ProcessSqlResult>& results) {
        return joinGeneratedCodeWithSpans(results).code;
    }
    ...
    JoinedGeneratedCode joinGeneratedCodeWithSpans(const std::vector<ProcessSqlResult>& results) {
        ...
        std::vector<PlacedFragment> structBlocks;
        std::vector<PlacedFragment> storageArguments;
        ...
        for (size_t index = 0; index < results.size(); ++index) {
            ...
            if (markerPosition != std::string::npos && code.ends_with(");")) {
                ...
                if (!structPart.empty()) {
                    structBlocks.push_back(PlacedFragment{std::move(structPart), origin});
                }
                storageArguments.push_back(
                    PlacedFragment{code.substr(markerPosition + storageMarker.size(),
        ...
        if (!storageArguments.empty()) {
            ...
            for (const PlacedFragment& storageArgument: storageArguments) {
                out.append(",\n    ");
                out.appendFragment(storageArgument.text, storageArgument.origin);
            }
            out.append(");\n");
        }
        bool separatorAdded = out.empty();
        for (const CodeSpanBuilder& statement: statementBlocks) {
            if (!separatorAdded) {
                out.append("\n");
                separatorAdded = true;
            }
            out.appendBuilt(statement);
            if (out.code().back() != '\n') {
                out.append("\n");
            }
        }
