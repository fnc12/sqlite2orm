    ...
    std::vector<ValidationError> Validator::validate(const AstNode& astNode) {
        ...
        if (auto* unaryOp = dynamic_cast<const UnaryOperatorNode*>(&astNode)) {
            ...
            if (unaryOp->unaryOperator == UnaryOperator::minus) {
                ...
                if (auto* literal = dynamic_cast<const IntegerLiteralNode*>(&withoutUnaryPluses(*unaryOp->operand))) {
                    ...
                    if (hexLiteralIsInt64Min(literal->value)) {
                        errors.push_back(
                            ValidationError{"hex literal too big: -" + withoutDigitSeparators(literal->value),
                                            unaryOp->location,
                                            "UnaryOperatorNode"});
                    }
                }
            ...
            if (!node->tableName.empty()) {
                std::string normalizedInTable = normalizeSqlName(node->tableName);
                if (this->knownCteTableNames.find(normalizedInTable) == this->knownCteTableNames.end()) {
                    errors.push_back(
                        ValidationError{"IN table-name is not supported in sqlite_orm", node->location, "InNode"});
                }
            ...
            if (createTable->columns.empty()) {
                errors.push_back(ValidationError{"CREATE TABLE must have at least one column",
            ...
            for (const auto& col: createTable->columns) {
                ...
                if (!seen.insert(lower).second) {
                    errors.push_back(ValidationError{"duplicate column name: " + col.name,
                                                     createTable->location,
                                                     "CreateTableNode"});
                }
                if (col.autoincrement && !col.primaryKey) {
                    errors.push_back(ValidationError{"AUTOINCREMENT requires PRIMARY KEY on column: " + col.name,
                                                     createTable->location,
                                                     "CreateTableNode"});
                }
            }
            ...
            for (const auto& cte: withNode->clause.tables) {
                this->knownCteTableNames.insert(normalizeSqlName(cte.cteName));
            }
            for (const auto& cte: withNode->clause.tables) {
            ...
            for (const auto& fromItem: selectNode->fromClause) {
                if (fromItem.table.derivedSelect) {
                    errors.push_back(ValidationError{"subselect in FROM is not supported in sqlite_orm",
                ...
                if (!fromItem.table.tableFunctionArgs.empty()) {
                    errors.push_back(
                        ValidationError{"table-valued function in FROM is not supported in sqlite_orm codegen",
                                        selectNode->location,
                                        "SelectNode"});
                ...
                if (fromItem.leadingJoin == JoinKind::naturalLeftJoin) {
                    errors.push_back(ValidationError{"NATURAL LEFT JOIN is not supported in sqlite_orm",
                                                     selectNode->location,
                                                     "SelectNode"});
                }
                const bool needsOnOrUsing =
                    fromItem.leadingJoin == JoinKind::innerJoin || fromItem.leadingJoin == JoinKind::joinPlain ||
                    fromItem.leadingJoin == JoinKind::leftJoin || fromItem.leadingJoin == JoinKind::leftOuterJoin;
                if (needsOnOrUsing && !fromItem.onExpression && fromItem.usingColumnNames.empty()) {
                    errors.push_back(
                        ValidationError{"JOIN requires ON or USING clause", selectNode->location, "SelectNode"});
                }
            ...
            if (updateNode->assignments.empty()) {
                errors.push_back(
                    ValidationError{"UPDATE requires at least one SET assignment", updateNode->location, "UpdateNode"});
            }
            ...
            if (!updateNode->fromClause.empty()) {
                errors.push_back(ValidationError{"UPDATE ... FROM ... is not supported in sqlite_orm",
                                                 updateNode->location,
                                                 "UpdateNode"});
            }
            ...
            if (raiseNode->message) {
                ...
                errors.push_back(ValidationError{"RAISE(ROLLBACK|ABORT|FAIL) requires a message expression",
                                                 raiseNode->location,
                                                 "RaiseNode"});
            }
        } else if (auto* createTrigger = dynamic_cast<const CreateTriggerNode*>(&astNode)) {
            if (createTrigger->bodyStatements.empty()) {
                errors.push_back(ValidationError{"CREATE TRIGGER body must contain at least one statement",
            ...
            if (createIndex->indexedColumns.empty()) {
                errors.push_back(ValidationError{"CREATE INDEX requires at least one indexed column",
            ...
            if (mod == "fts5" && cvt->moduleArguments.empty()) {
                errors.push_back(ValidationError{"FTS5 virtual table requires at least one module argument (column)",
                                                 cvt->location,
                                                 "CreateVirtualTableNode"});
            }
            if (mod == "dbstat" && cvt->moduleArguments.size() > 1) {
                errors.push_back(ValidationError{"dbstat virtual table accepts at most one optional schema argument",
                                                 cvt->location,
                                                 "CreateVirtualTableNode"});
            }
            if (mod == "rtree" || mod == "rtree_i32") {
                ...
                if (n == 0) {
                    errors.push_back(ValidationError{"RTREE virtual table requires module column arguments",
                                                     cvt->location,
                                                     "CreateVirtualTableNode"});
                } else {
                    bool allCol = true;
                    ...
                    if (allCol && (n < 3 || n > 11 || (n % 2 == 0))) {
                        errors.push_back(
                            ValidationError{"RTREE virtual table must declare 3, 5, 7, 9, or 11 column arguments",
                                            cvt->location,
                                            "CreateVirtualTableNode"});
                    }
                }
            ...
            errors.push_back(
                ValidationError{"ALTER TABLE " + display +
            ...
            errors.push_back(ValidationError{"ATTACH DATABASE is not supported in sqlite_orm",
                                             node->location,
                                             "AttachDatabaseNode"});
        } else if (auto* node = dynamic_cast<const DetachDatabaseNode*>(&astNode)) {
            errors.push_back(ValidationError{"DETACH DATABASE is not supported in sqlite_orm",
                                             node->location,
                                             "DetachDatabaseNode"});
        } else if (auto* node = dynamic_cast<const AnalyzeNode*>(&astNode)) {
            errors.push_back(ValidationError{"ANALYZE is not supported in sqlite_orm", node->location, "AnalyzeNode"});
        } else if (auto* node = dynamic_cast<const ReindexNode*>(&astNode)) {
            errors.push_back(ValidationError{"REINDEX is not supported in sqlite_orm", node->location, "ReindexNode"});
            ...
            errors.push_back(ValidationError{"EXPLAIN is not supported in sqlite_orm", node->location, "ExplainNode"});
            if (node->statement) {
        ...
        auto validateReturning =
            [&](const std::vector<ReturningColumn>& returning, SourceLocation location, const char* nodeType) {
                if (!returning.empty()) {
                    errors.push_back(
                        ValidationError{"RETURNING clause is not supported in sqlite_orm", location, nodeType});
                }
            };
        ...
        auto validateNullsOrdering = [&](const std::vector<OrderByTerm>& orderBy,
            ...
            for (const auto& term: orderBy) {
                if (term.nulls != NullsOrdering::none) {
                    errors.push_back(
                        ValidationError{"NULLS FIRST / NULLS LAST is not supported in sqlite_orm", location, nodeType});
                    break;
                }
            }
        };
