...
TEST_CASE("codegen: warnings from a subquery LIMIT expression reach the statement") {
    REQUIRE(generateFull("SELECT (SELECT name FROM users LIMIT 1 COLLATE NOCASE) FROM users") ==
            CodeGenResult{"auto rows = storage.select(select(&Users::name, limit(1)), from<Users>());",
                          {columnRefStyleDp(1, "&Users::name")},
                          {"COLLATE NOCASE on expressions is not directly supported in sqlite_orm codegen"}});
}
...
// and only the three-argument form propagates a NULL argument and nothing else. The nullability
// rule is written over the arity for that reason; what a short form generates is now the
// expressibility gate's answer, because sqlite_orm declares the three-argument `iif` alone
// (pinned in "codegen: a built-in written with an argument count sqlite_orm has no overload for is
// not generated").
TEST_CASE("codegen: an iif in its three-argument form is not widened") {
...
// table at all, answering one row where SQLite answers one per row of the table (card
// 1868936178586092581). What a subquery names is not a FROM this level can infer from: only the
// mentions a select makes itself are. Row counts checked against sqlite3 3.51 in
// `codegen_tests_runtime_values.cpp`.
TEST_CASE("codegen: a select whose subquery names its own table carries its FROM") {
    ...
    REQUIRE(generate("SELECT 1 FROM users WHERE EXISTS (SELECT 1 FROM users) LIMIT 2;") ==
            "auto rows = storage.select(1, from<Users>(), where(exists(select(1, from<Users>()))), limit(2));");
    REQUIRE(generate("SELECT 1 FROM orders WHERE EXISTS (SELECT 1 FROM orders);") ==
            "auto rows = storage.select(1, from<Orders>(), where(exists(select(1, from<Orders>()))));");
    REQUIRE(generate("SELECT 1 FROM users u WHERE EXISTS (SELECT 1 FROM users);") ==
            "auto rows = storage.select(1, from<alias_a<Users>>(), where(exists(select(1, from<Users>()))));");
}

// The mentions of the subquery still answer for the width of this FROM, so the sources it names
// are the ones sqlite_orm has to infer and no more: a joined table arrives through its own clause,
// and every level of a nest gets the FROM its own code leaves it without.
TEST_CASE("codegen: a select whose subquery names its own table names the sources it infers") {
    REQUIRE(generate("SELECT 1 FROM users, orders WHERE EXISTS (SELECT 1 FROM users);") ==
            "auto rows = storage.select(1, from<Users>(), cross_join<Orders>(), "
            "where(exists(select(1, from<Users>()))));");
    REQUIRE(generate("SELECT 1 FROM users JOIN orders ON 1 WHERE EXISTS (SELECT 1 FROM orders);") ==
            "auto rows = storage.select(1, from<Users>(), join<Orders>(on(1)), "
            "where(exists(select(1, from<Orders>()))));");
    REQUIRE(generate("SELECT 1 FROM users GROUP BY 1 HAVING EXISTS (SELECT 1 FROM users);") ==
            "auto rows = storage.select(1, from<Users>(), group_by(1).having(exists(select(1, from<Users>()))));");
    REQUIRE(generate("SELECT 1 FROM users WHERE EXISTS (SELECT 1 FROM users WHERE EXISTS (SELECT 1 FROM users));") ==
            "auto rows = storage.select(1, from<Users>(), where(exists(select(1, from<Users>(), "
            "where(exists(select(1, from<Users>())))))));");
    REQUIRE(generate("INSERT INTO orders SELECT 1 FROM users WHERE EXISTS (SELECT 1 FROM users);") ==
            "storage.insert(into<Orders>(), select(1, from<Users>(), where(exists(select(1, from<Users>())))));");
}

// A mention a subquery makes with a column of its own is one this FROM covers, and the statement
// used to lean on it: sqlite_orm collected it into the outer FROM and the rows came out right by
// accident. The FROM is written out all the same — what the subquery names answers for its own
// level.
TEST_CASE("codegen: a select leaning on a mention its subquery made carries its FROM") {
    REQUIRE(generate("SELECT 1 FROM users WHERE EXISTS (SELECT a FROM users);") ==
            "auto rows = storage.select(1, from<Users>(), where(exists(select(&Users::a))));");
    REQUIRE(generate("SELECT 1 FROM users ORDER BY (SELECT a FROM users LIMIT 1);") ==
