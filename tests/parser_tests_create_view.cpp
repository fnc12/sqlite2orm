...
namespace {
    ...
    AstNodePointer selectLiteralOne() {
        ...
        sel->columns = {SelectColumn{makeSharedNode<IntegerLiteralNode>("1"), ""}};
        return sel;
    }
    ...
    AstNodePointer selectLiteralsOneTwo() {
        auto sel = std::make_unique<SelectNode>(SourceLocation{});
        sel->columns = {SelectColumn{makeSharedNode<IntegerLiteralNode>("1"), ""},
                        SelectColumn{makeSharedNode<IntegerLiteralNode>("2"), ""}};
        return sel;
    }
...
TEST_CASE("parser: CREATE VIEW") {
    ...
    REQUIRE(requireNode<CreateViewNode>(parseResult) == expected);
}
