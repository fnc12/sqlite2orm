...
TEST_CASE("parser: qualified column ref") {
    ...
    SECTION("quoted table and column") {
        ...
        REQUIRE(requireNode<QualifiedColumnRefNode>(parseResult) ==
                QualifiedColumnRefNode(R"("my table")", R"("my column")", {}));
    }
    SECTION("schema.table.column") {
        auto parseResult = parse("main.users.id");
        REQUIRE(requireNode<QualifiedColumnRefNode>(parseResult) == QualifiedColumnRefNode("main", "users", "id", {}));
    }
}
...
TEST_CASE("parser: window function OVER") {
    ...
    expected.over->orderBy.push_back(OrderByTerm{makeSharedNode<ColumnRefNode>("b"), SortDirection::desc});
...
TEST_CASE("parser: aggregate FILTER (WHERE …) before OVER") {
    ...
    expected.filterWhere = std::make_unique<BinaryOperatorNode>(BinaryOperator::greaterThan,
...
TEST_CASE("parser: binary comparison") {
    SECTION("equals") {
        ...
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::equals,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    ...
    SECTION("not equals !=") {
        auto parseResult = parse("a != 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::notEquals,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    ...
    SECTION("less than") {
        auto parseResult = parse("a < 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::lessThan,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("less or equal") {
        auto parseResult = parse("a <= 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::lessOrEqual,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("greater than") {
        auto parseResult = parse("a > 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::greaterThan,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("greater or equal") {
        auto parseResult = parse("a >= 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::greaterOrEqual,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("column vs string") {
        ...
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
                BinaryOperatorNode(BinaryOperator::equals,
                                   makeNode<ColumnRefNode>("name"),
                                   makeNode<StringLiteralNode>("'hello'"),
                                   {}));
    }
    SECTION("qualified column ref as operand") {
        ...
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
                BinaryOperatorNode(BinaryOperator::equals,
                                   makeNode<QualifiedColumnRefNode>("users", "id"),
                                   makeNode<IntegerLiteralNode>("42"),
                                   {}));
    }
...
TEST_CASE("parser: arithmetic operators") {
    SECTION("add") {
        auto parseResult = parse("a + 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::add,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("subtract") {
        auto parseResult = parse("a - 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::subtract,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("multiply") {
        auto parseResult = parse("a * 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::multiply,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("divide") {
        auto parseResult = parse("a / 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::divide,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("modulo") {
        auto parseResult = parse("a % 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::modulo,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
...
TEST_CASE("parser: concatenation") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::concatenate,
                                                                               makeNode<ColumnRefNode>("a"),
...
TEST_CASE("parser: bitwise operators") {
    SECTION("bitwise and") {
        auto parseResult = parse("a & 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::bitwiseAnd,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("bitwise or") {
        auto parseResult = parse("a | 5");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::bitwiseOr,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("5"),
                                                                                   {}));
    }
    SECTION("shift left") {
        ...
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::shiftLeft,
                                                                                   makeNode<ColumnRefNode>("a"),
    ...
    SECTION("shift right") {
        auto parseResult = parse("a >> 2");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) == BinaryOperatorNode(BinaryOperator::shiftRight,
                                                                                   makeNode<ColumnRefNode>("a"),
                                                                                   makeNode<IntegerLiteralNode>("2"),
                                                                                   {}));
    }
...
TEST_CASE("parser: unary operators") {
    SECTION("unary minus on literal") {
        auto parseResult = parse("-5");
        REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
                UnaryOperatorNode(UnaryOperator::minus, makeNode<IntegerLiteralNode>("5"), {}));
    }
    SECTION("unary minus on column") {
        auto parseResult = parse("-a");
        REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
                UnaryOperatorNode(UnaryOperator::minus, makeNode<ColumnRefNode>("a"), {}));
    }
    SECTION("unary plus") {
        auto parseResult = parse("+5");
        REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
                UnaryOperatorNode(UnaryOperator::plus, makeNode<IntegerLiteralNode>("5"), {}));
    }
    SECTION("bitwise not") {
        auto parseResult = parse("~5");
        REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
                UnaryOperatorNode(UnaryOperator::bitwiseNot, makeNode<IntegerLiteralNode>("5"), {}));
    }
    SECTION("double unary minus") {
        auto parseResult = parse("- -5");
        REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
                UnaryOperatorNode(UnaryOperator::minus,
                                  std::make_unique<UnaryOperatorNode>(UnaryOperator::minus,
                                                                      makeNode<IntegerLiteralNode>("5"),
                                                                      SourceLocation{}),
                                  {}));
    }
...
TEST_CASE("parser: operator precedence") {
    SECTION("multiply before add: a + b * c") {
        ...
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
                BinaryOperatorNode(BinaryOperator::add,
                                   makeNode<ColumnRefNode>("a"),
    ...
    SECTION("binary minus with unary minus operand: a - -b") {
        auto parseResult = parse("a - -b");
        REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
                BinaryOperatorNode(BinaryOperator::subtract,
                                   makeNode<ColumnRefNode>("a"),
                                   std::make_unique<UnaryOperatorNode>(UnaryOperator::minus,
                                                                       makeNode<ColumnRefNode>("b"),
                                                                       SourceLocation{}),
...
TEST_CASE("parser: logical AND") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
            BinaryOperatorNode(BinaryOperator::logicalAnd,
...
TEST_CASE("parser: logical OR") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
            BinaryOperatorNode(BinaryOperator::logicalOr,
                               std::make_unique<BinaryOperatorNode>(BinaryOperator::equals,
                                                                    makeNode<ColumnRefNode>("a"),
                                                                    makeNode<IntegerLiteralNode>("1"),
                                                                    SourceLocation{}),
...
TEST_CASE("parser: logical NOT") {
    ...
    REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
            UnaryOperatorNode(UnaryOperator::logicalNot, makeNode<ColumnRefNode>("a"), {}));
}
...
TEST_CASE("parser: NOT binds tighter than AND") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
            BinaryOperatorNode(BinaryOperator::logicalAnd,
                               std::make_unique<UnaryOperatorNode>(UnaryOperator::logicalNot,
                                                                   makeNode<ColumnRefNode>("a"),
                                                                   SourceLocation{}),
                               makeNode<ColumnRefNode>("b"),
                               {}));
...
TEST_CASE("parser: NOT is weaker than arithmetic") {
    ...
    REQUIRE(requireNode<UnaryOperatorNode>(parseResult) ==
            UnaryOperatorNode(UnaryOperator::logicalNot,
                              std::make_unique<BinaryOperatorNode>(BinaryOperator::add,
                                                                   makeNode<ColumnRefNode>("a"),
                                                                   makeNode<IntegerLiteralNode>("1"),
                                                                   SourceLocation{}),
...
TEST_CASE("parser: NOT is weaker than IN") {
    ...
    REQUIRE(
        requireNode<UnaryOperatorNode>(parseResult) ==
        UnaryOperatorNode(
            UnaryOperator::logicalNot,
            std::make_unique<InNode>(makeNode<ColumnRefNode>("a"), std::move(values), nullptr, false, SourceLocation{}),
            {}));
}

TEST_CASE("parser: IS NULL") {
    auto parseResult = parse("a IS NULL");
    REQUIRE(requireNode<IsNullNode>(parseResult) == IsNullNode(makeNode<ColumnRefNode>("a"), {}));
}

TEST_CASE("parser: IS NOT NULL") {
    auto parseResult = parse("a IS NOT NULL");
    REQUIRE(requireNode<IsNotNullNode>(parseResult) == IsNotNullNode(makeNode<ColumnRefNode>("a"), {}));
}
...
TEST_CASE("parser: BETWEEN") {
    ...
    REQUIRE(requireNode<BetweenNode>(parseResult) == BetweenNode(makeNode<ColumnRefNode>("a"),
                                                                 makeNode<IntegerLiteralNode>("1"),
...
TEST_CASE("parser: BETWEEN with AND precedence") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
            BinaryOperatorNode(BinaryOperator::logicalAnd,
                               std::make_unique<BetweenNode>(makeNode<ColumnRefNode>("a"),
                                                             makeNode<IntegerLiteralNode>("1"),
                                                             makeNode<IntegerLiteralNode>("10"),
                                                             false,
                                                             SourceLocation{}),
...
TEST_CASE("parser: IN with empty list") {
    ...
    REQUIRE(requireNode<InNode>(parseResult) == InNode(makeNode<ColumnRefNode>("a"), {}, nullptr, false, {}));
}
...
TEST_CASE("parser: LIKE with ESCAPE") {
    ...
    REQUIRE(requireNode<LikeNode>(parseResult) == LikeNode(makeNode<ColumnRefNode>("name"),
...
TEST_CASE("parser: function call - no args") {
    ...
    REQUIRE(requireNode<FunctionCallNode>(parseResult) == *makeFunc("random", false, false));
}

TEST_CASE("parser: function call - one arg") {
    auto parseResult = parse("abs(a)");
    REQUIRE(requireNode<FunctionCallNode>(parseResult) == *makeFunc("abs", false, false, makeNode<ColumnRefNode>("a")));
}

TEST_CASE("parser: function call - multiple args") {
    auto parseResult = parse("coalesce(a, b, 0)");
    REQUIRE(requireNode<FunctionCallNode>(parseResult) == *makeFunc("coalesce",
                                                                    false,
                                                                    false,
                                                                    makeNode<ColumnRefNode>("a"),
                                                                    makeNode<ColumnRefNode>("b"),
...
TEST_CASE("parser: function call - count(*)") {
    ...
    REQUIRE(requireNode<FunctionCallNode>(parseResult) == *makeFunc("count", false, true));
}
...
TEST_CASE("parser: function call - nested") {
    ...
    REQUIRE(
        requireNode<FunctionCallNode>(parseResult) ==
        *makeFunc("abs",
                  false,
                  false,
                  makeFunc("round", false, false, makeNode<ColumnRefNode>("x"), makeNode<IntegerLiteralNode>("2"))));
}
...
TEST_CASE("parser: keyword as function - replace") {
    ...
    REQUIRE(requireNode<FunctionCallNode>(parseResult) == *makeFunc("replace",
                                                                    false,
                                                                    false,
                                                                    makeNode<ColumnRefNode>("name"),
...
TEST_CASE("parser: function with IS NULL") {
    ...
    REQUIRE(requireNode<IsNullNode>(parseResult) ==
            IsNullNode(makeFunc("length", false, false, makeNode<ColumnRefNode>("name")), {}));
}
...
TEST_CASE("parser: CAST with simple type") {
    ...
    REQUIRE(requireNode<CastNode>(parseResult) == CastNode(makeNode<ColumnRefNode>("a"), "INTEGER", {}));
}

TEST_CASE("parser: CAST with type and size") {
    auto parseResult = parse("CAST(name AS VARCHAR(255))");
    REQUIRE(requireNode<CastNode>(parseResult) == CastNode(makeNode<ColumnRefNode>("name"), "VARCHAR(255)", {}));
}

TEST_CASE("parser: CAST with multi-word type") {
    auto parseResult = parse("CAST(x AS UNSIGNED BIG INT)");
    REQUIRE(requireNode<CastNode>(parseResult) == CastNode(makeNode<ColumnRefNode>("x"), "UNSIGNED BIG INT", {}));
}

TEST_CASE("parser: CAST with expression operand") {
    auto parseResult = parse("CAST(a + b AS REAL)");
    REQUIRE(requireNode<CastNode>(parseResult) == CastNode(makeNode<BinaryOperatorNode>(BinaryOperator::add,
                                                                                        makeNode<ColumnRefNode>("a"),
                                                                                        makeNode<ColumnRefNode>("b")),
                                                           "REAL",
                                                           {}));
...
TEST_CASE("parser: searched CASE") {
    ...
    branches.push_back(CaseBranch{makeNode<BinaryOperatorNode>(BinaryOperator::greaterThan,
                                                               makeNode<ColumnRefNode>("a"),
                                                               makeNode<IntegerLiteralNode>("0")),
...
TEST_CASE("parser: CASE with multiple WHEN and ELSE") {
    ...
    std::vector<CaseBranch> branches;
    branches.push_back(CaseBranch{makeNode<BinaryOperatorNode>(BinaryOperator::equals,
                                                               makeNode<ColumnRefNode>("a"),
                                                               makeNode<IntegerLiteralNode>("1")),
...
TEST_CASE("parser: IS expr") {
    ...
    SelectNode expected({});
    expected.columns = {SelectColumn{std::make_shared<BinaryOperatorNode>(BinaryOperator::isOp,
                                                                          makeNode<IntegerLiteralNode>("1"),
                                                                          makeNode<IntegerLiteralNode>("2"),
                                                                          SourceLocation{}),
                                     ""}};
...
TEST_CASE("parser: IS NOT expr") {
    ...
    SelectNode expected({});
    expected.columns = {SelectColumn{std::make_shared<BinaryOperatorNode>(BinaryOperator::isNot,
                                                                          makeNode<ColumnRefNode>("a"),
                                                                          makeNode<ColumnRefNode>("b"),
                                                                          SourceLocation{}),
                                     ""}};
...
TEST_CASE("parser: IS DISTINCT FROM") {
    ...
    SelectNode expected({});
    expected.columns = {SelectColumn{std::make_shared<BinaryOperatorNode>(BinaryOperator::isDistinctFrom,
                                                                          makeNode<ColumnRefNode>("a"),
                                                                          makeNode<ColumnRefNode>("b"),
                                                                          SourceLocation{}),
                                     ""}};
...
TEST_CASE("parser: IS NOT DISTINCT FROM") {
    ...
    SelectNode expected({});
    expected.columns = {SelectColumn{std::make_shared<BinaryOperatorNode>(BinaryOperator::isNotDistinctFrom,
                                                                          makeNode<ColumnRefNode>("a"),
                                                                          makeNode<ColumnRefNode>("b"),
                                                                          SourceLocation{}),
                                     ""}};
...
TEST_CASE("parser: IS NULL is not an atomic postfix") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
            BinaryOperatorNode(BinaryOperator::isOp,
                               makeNode<ColumnRefNode>("a"),
                               std::make_unique<BinaryOperatorNode>(BinaryOperator::subtract,
                                                                    makeNode<NullLiteralNode>(),
                                                                    makeNode<IntegerLiteralNode>("1"),
                                                                    SourceLocation{}),
                               {}));
}
...
TEST_CASE("parser: IS NOT NULL is not an atomic postfix") {
    ...
    REQUIRE(requireNode<BinaryOperatorNode>(parseResult) ==
            BinaryOperatorNode(BinaryOperator::isNot,
                               makeNode<ColumnRefNode>("a"),
                               std::make_unique<BinaryOperatorNode>(BinaryOperator::subtract,
                                                                    makeNode<NullLiteralNode>(),
                                                                    makeNode<IntegerLiteralNode>("1"),
...
TEST_CASE("parser: IS NULL keeps the '=' level to its left") {
    ...
    REQUIRE(requireNode<IsNullNode>(parseResult) ==
            IsNullNode(std::make_unique<BinaryOperatorNode>(BinaryOperator::add,
                                                            makeNode<ColumnRefNode>("a"),
                                                            makeNode<IntegerLiteralNode>("1"),
                                                            SourceLocation{}),
                       {}));
...
TEST_CASE("parser: IS NULL is left-associative with itself") {
    ...
    REQUIRE(requireNode<IsNullNode>(parseResult) ==
            IsNullNode(std::make_unique<IsNullNode>(makeNode<ColumnRefNode>("a"), SourceLocation{}), {}));
}
...
TEST_CASE("parser: JSON -> operator") {
    ...
    REQUIRE(*binOp == BinaryOperatorNode(BinaryOperator::jsonArrow,
                                         makeNode<ColumnRefNode>("data"),
                                         makeNode<StringLiteralNode>("'$.name'"),
                                         {}));
}
...
TEST_CASE("parser: JSON ->> operator") {
    ...
    auto* binOp = dynamic_cast<const BinaryOperatorNode*>(sel.columns.at(0).expression.get());
    REQUIRE(binOp != nullptr);
    REQUIRE(*binOp == BinaryOperatorNode(BinaryOperator::jsonArrow2,
                                         makeNode<ColumnRefNode>("data"),
                                         makeNode<StringLiteralNode>("'$.name'"),
                                         {}));
}
