...
TEST_CASE("parser: SAVEPOINT name") {
    ...
    require_node<SavepointNode>(result);
    REQUIRE(require_node<SavepointNode>(result) == SavepointNode({}, "sp1"));
}

TEST_CASE("parser: RELEASE SAVEPOINT name") {
    auto result = parse("RELEASE SAVEPOINT sp1;");
    REQUIRE(require_node<ReleaseNode>(result) == ReleaseNode({}, "sp1"));
}
...
TEST_CASE("parser: ATTACH DATABASE") {
    ...
    const auto& node = require_node<AttachDatabaseNode>(result);
...
TEST_CASE("parser: DETACH DATABASE") {
    ...
    REQUIRE(require_node<DetachDatabaseNode>(result) == DetachDatabaseNode({}, "aux"));
}
...
TEST_CASE("parser: ANALYZE bare") {
    ...
    REQUIRE(require_node<AnalyzeNode>(result) == expected);
}
...
TEST_CASE("parser: REINDEX bare") {
    ...
    REQUIRE(require_node<ReindexNode>(result) == expected);
}
...
TEST_CASE("parser: PRAGMA query") {
    ...
    REQUIRE(require_node<PragmaNode>(result) == expected);
}

TEST_CASE("parser: PRAGMA with value") {
    auto result = parse("PRAGMA journal_mode = WAL;");
    const auto& node = require_node<PragmaNode>(result);
...
TEST_CASE("parser: EXPLAIN SELECT") {
    ...
    const auto& node = require_node<ExplainNode>(result);
...
TEST_CASE("parser: ORDER BY with NULLS FIRST") {
    ...
    const auto& node = require_node<SelectNode>(result);
...
TEST_CASE("parser: INSERT with RETURNING") {
    ...
    const auto& node = require_node<InsertNode>(result);
...
TEST_CASE("parser: UPDATE with RETURNING *") {
    ...
    const auto& node = require_node<UpdateNode>(result);
...
TEST_CASE("parser: DELETE with RETURNING") {
    ...
    expectedAst->whereClause = make_node<BinaryOperatorNode>(BinaryOperator::equals, make_node<ColumnRefNode>("id"),
                                                             make_node<IntegerLiteralNode>("1"));
    expectedAst->returning.push_back(ReturningColumn{make_node<ColumnRefNode>("id"), ""});
